    ...
    >
      <div className="relative w-full max-w-[960px] max-h-[92vh] overflow-y-auto bg-white shadow-2xl flex flex-col">
        ...
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 rounded transition-all"
        >
        ...
        {/* Dark Header */}
        <div className="bg-[#1a1a1a] px-8 sm:px-10 pt-6 pb-5">
        ...
        {/* Step Tabs */}
        <div className="border-b border-neutral-200 px-8 sm:px-10">
          <div className="flex">
            {stepLabels.map((label, i) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 py-4 text-left transition-all rounded ${
                    isDone ? "cursor-pointer hover:bg-neutral-50" : "cursor-default"
                  }`}
                >
                  <span
                    className={`font-sans text-[11px] sm:text-xs tracking-[0.1em] uppercase font-semibold ${
                      isActive
                        ? "text-[#C0532C]"
                        : isDone
                        ? "text-[#C0532C]"
        ...
        <div className="flex-1 overflow-y-auto px-8 sm:px-10 py-8">
          {step === 1 && (
            <div>
              <h2 className="font-serif text-[#1a1a1a] text-3xl sm:text-4xl font-normal leading-tight mb-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                {[
                  ...
                  <div key={name}>
                    <label className="block font-sans text-[11px] font-semibold tracking-[0.12em] text-neutral-700 mb-2">
                    ...
                    <input
                      ...
                      className="w-full h-12 border border-neutral-200 px-3 text-sm text-neutral-800 placeholder-neutral-300 font-sans focus:outline-none focus:border-[#C0532C] transition-colors rounded-none"
                    />
                  </div>
                ))}
              </div>
          ...
          {step === 2 && (
            <div>
              ...
              <label className="block font-sans text-[11px] font-semibold tracking-[0.12em] text-neutral-700 mb-3">
                ...
                {spaceTypes.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-4 border transition-all rounded ${
                        active
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-sm font-sans font-medium ${active ? "text-[#C0532C]" : "text-neutral-700"}`}>
                          {t}
                        </span>
                      </div>
                    </button>
                  ...
                  <div key={name}>
                    ...
                    <div className="relative">
                      <select
                        ...
                        value={form[name]}
                        onChange={handleChange}
                        className="w-full h-12 border border-neutral-200 px-3 pr-8 text-sm text-neutral-700 font-sans bg-white focus:outline-none focus:border-[#C0532C] appearance-none rounded-none cursor-pointer"
                      >
                        {opts.map((o) => (
                ...
                {timelineOptions.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`w-4 h-4 flex-shrink-0 flex items-center justify-center border rounded-full transition-colors ${
                            active ? "border-[#C0532C]" : "border-neutral-300"
                          }`}
                        >
                          {active && (
                            <span className="w-2 h-2 bg-[#C0532C] rounded-full" />
                          )}
                        </span>
                        <span className={`text-sm font-sans font-medium leading-tight ${active ? "text-[#C0532C]" : "text-neutral-700"}`}>
                          {t}
                        </span>
                      </div>
                    </button>
                ...
                {serviceOptions.map((svc) => {
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-2 px-4 py-2.5 border text-sm font-sans font-medium transition-all rounded ${
                        active
        ...
        <div className="border-t border-neutral-200 px-8 sm:px-10 py-5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {step === 1 && (
              <span className="w-1.5 h-1.5 bg-[#C0532C] rounded-full" />
            )}
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 h-11 border border-neutral-300 font-sans text-[11px] tracking-[0.14em] uppercase font-semibold text-neutral-700 hover:bg-neutral-100 transition-all rounded"
            >
              {step === 1 ? "CANCEL" : "BACK"}
            </button>
            <button
              ...
              disabled={!canNext}
              className={`h-11 px-8 font-sans text-[11px] tracking-[0.14em] uppercase font-semibold transition-all rounded ${
                step === 3
