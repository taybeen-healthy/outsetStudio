...
function StatItem({ stat, inView, index }) {
  return (
    ...
    >
      <span
        className={`font-serif tabular-nums text-4xl sm:text-5xl lg:text-[56px] font-light leading-none tracking-tight transition-colors duration-500 ${
    ...
    return (
      <div ref={ref} aria-label="Key statistics" className="grid grid-cols-2 gap-2.5">
        {stats.map((stat, i) => (
          ...
          >
            <span
              className={`font-serif tabular-nums text-[32px] font-normal leading-none tracking-tight ${
                stat.highlight ? "text-[#C0532C]" : "text-[#1a1a1a]"
              }`}
            >
              <CountUp value={stat.value} inView={inView} />
